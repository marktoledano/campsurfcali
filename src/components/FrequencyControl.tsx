...
export function FrequencyControl({ watches, onAppliedToAll }: Props) {
  const allSame = watches.every((w) => w.checkFrequencyMinutes === watches[0].checkFrequencyMinutes)
  const initial = allSame ? minutesToFrequency(watches[0].checkFrequencyMinutes) : { value: 5, unit: 'minutes' as FrequencyUnit }

  const [value, setValue] = useState(initial.value)
  const [unit, setUnit] = useState<FrequencyUnit>(initial.unit)
  ...
  async function apply() {
    ...
    setDone(false)
    try {
      const updated = await api.setFrequencyForAll(frequencyToMinutes(value, unit))
  ...
  return (
    <div className="flex flex-wrap items-center gap-3 rounded-2xl border border-pine/10 bg-paper/70 px-5 py-4">
      ...
      <span className="text-xs text-pine-soft">
